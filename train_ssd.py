...
parser.add_argument(
    "--balance-data", action="store_true", help="Balance training data by down-sampling more frequent labels."
)
...
parser.add_argument("--mb2-width-mult", default=1.0, type=float, help="Width Multiplifier for MobilenetV2")

# Params for loading pretrained basenet or checkpoints.
parser.add_argument("--base-net", help="Pretrained base model")
parser.add_argument(
    "--pretrained-ssd", default="models/mobilenet-v1-ssd-mp-0_675.pth", type=str, help="Pre-trained base model"
)
...
parser.add_argument(
    "--base-net-lr", default=0.001, type=float, help="initial learning rate for base net, or None to use --lr"
)
parser.add_argument(
    "--extra-layers-lr",
    default=None,
...
# Scheduler
parser.add_argument(
    "--scheduler", default="cosine", type=str, help="Scheduler for SGD. It can one of multi-step and cosine"
)
...
logging.basicConfig(
    stream=sys.stdout, level=logging.INFO, format="%(asctime)s - %(message)s", datefmt="%Y-%m-%d %H:%M:%S"
)
...
if __name__ == "__main__":
    ...
    logging.info(args)
    # Initialize wandb
    wandb.init(project="vision_ssd_project", entity="krishnachaitanya7")
    ...
    if args.dataset_type == "voc":
        val_dataset = VOCDataset(
            dataset_path, transform=test_transform, target_transform=target_transform, is_test=True
        )
    elif args.dataset_type == "open_images":
        val_dataset = OpenImagesDataset(
            dataset_path, transform=test_transform, target_transform=target_transform, dataset_type="test"
        )
    ...
    criterion = MultiboxLoss(
        config.priors, iou_threshold=0.5, neg_pos_ratio=3, center_variance=0.1, size_variance=0.2, device=DEVICE
    )
